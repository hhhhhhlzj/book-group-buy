...
import com.shuxiang.groupbuy.domain.order.adapter.port.IProductPort;
import com.shuxiang.groupbuy.domain.order.adapter.repository.IOrderRepository;
import com.shuxiang.groupbuy.domain.order.model.aggregate.CreateOrderAggregate;
import com.shuxiang.groupbuy.domain.order.model.entity.*;
...
public abstract class AbstractOrderService implements IOrderService {
    ...
    public PayOrderEntity createOrder(ShopCartEntity shopCartEntity) throws Exception {
        ...
        if (null != unpaidOrderEntity && OrderStatusVO.PAY_WAIT.equals(unpaidOrderEntity.getOrderStatusVO())) {
            // 不再返回库里旧 payUrl（含旧 timestamp），否则手机扫码常提示「二维码失效」；按当前订单信息重新签名唤起支付
            log.info("创建订单-存在，已存在未支付订单，重新唤起支付单。userId:{} productId:{} orderId:{}", shopCartEntity.getUserId(), shopCartEntity.getProductId(), unpaidOrderEntity.getOrderId());
        ...
        CreateOrderAggregate orderAggregate = CreateOrderAggregate.builder()
                ...
                .addressId(shopCartEntity.getAddressId())
                .receiverName(shopCartEntity.getReceiverName())
                .receiverPhone(shopCartEntity.getReceiverPhone())
                .receiverRegion(shopCartEntity.getReceiverRegion())
                .receiverDetail(shopCartEntity.getReceiverDetail())
                .build();
