...
import com.shuxiang.groupbuy.domain.activity.model.valobj.*;
import com.shuxiang.groupbuy.infrastructure.dao.*;
import com.shuxiang.groupbuy.infrastructure.dao.po.*;
...
public class ActivityRepository extends AbstractRepository implements IActivityRepository {
    ...
    @Override
    public List<SkuVO> querySkuListBySourceChannel(String source, String channel) {
        List<Sku> skuList = skuDao.querySkuListBySourceChannel(source, channel);
        if (null == skuList || skuList.isEmpty()) {
            return Collections.emptyList();
        }
        return skuList.stream()
                .map(sku -> SkuVO.builder()
                        .goodsId(sku.getGoodsId())
                        .goodsName(sku.getGoodsName())
                        .originalPrice(sku.getOriginalPrice())
                        .build())
